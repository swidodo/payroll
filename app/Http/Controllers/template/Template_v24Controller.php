...
class Template_v24Controller extends Controller
{
        ...
        foreach ($sheetData as $key => $value) {
            if ($key > 0) :
                ...
                if ($branch != null) :
                    ...
                    $ded_other = Deduction_other::where('employee_id',$employeeId->id)->where('date',$value[14])->count();
                    if($ded_other > 0 ){
                        Deduction_other::where('employee_id',$employeeId->id)->where('date',$value[14])->delete();
                    }
                    $ded_loan = Loan::where('employee_id',$employeeId->id)->where(DB::raw("DATE(created_at)"),$value[14])->count();
                    if ($ded_loan > 0 ){
                        Loan::where('employee_id',$employeeId->id)->where(DB::raw("DATE(created_at)"),$value[14])->delete();
                    }
                    $allFinance = AllowanceFinance::where('employee_id',$employeeId->id)->where(DB::raw("DATE(created_at)"),$value[14])->count();
                    if ($allFinance > 0){
                        AllowanceFinance::where('employee_id',$employeeId->id)->where(DB::raw("DATE(created_at)"),$value[14])->delete();
                    }
                    $allother = DB::table('allowances')->where('employee_id',$employeeId->id)->where('date',$value[14])->count();
                    if ($allother > 0){
                        DB::table('allowances')->where('employee_id',$employeeId->id)->where(DB::raw("DATE(created_at)"),$value[14])->delete();
                    }
                    if ($employeeId != null ):
                        ...
                        $allowancefixed   =  0;
                        $allowanceUnfixed =  (($value[6] !=null) ? $value[6] : 0 ) + (($value[7] !=null) ? $value[7] : 0 );
                        $overtime         =  (($value[8] !=null) ? $value[8] : 0 );
                        $val_salarymonth  =  $basic_salary +  $allowanceUnfixed +  $overtime;

                        $total_loan       = 0;
                        $deduction_other  = (($value[9] !=null) ? $value[9] : 0 ) + (($value[10] !=null) ? $value[10] : 0 );
                        ...
                        if ($value[6] !=null){
                            $opt = AllowanceOption::where('name','Tunjangan Kesejahteraan')->where('pay_type','unfixed')->where('include_attendance','N')->first();
                            if ($opt !=null){
                                $data =[
                                    'employee_id'       => $employeeId->id,
                                    ...
                                    'date'              => date('Y-m-d', strtotime($value[14])),
                                    'branch_id'          => $employeeId->branch_id,
                        ...
                        if ($value[7] !=null){
                            $opt = AllowanceOption::where('name','Tunjangan Harian')->where('pay_type','unfixed')->where('include_attendance','N')->first();
                            if ($opt !=null){
                                $data =[
                                    'employee_id'       => $employeeId->id,
                                    'allowance_option_id' => $opt->id,
