...
class PayrollController extends Controller
{
    ...
    public function generate_run_payroll(Request $request){
        try {
            ...
            $pph = DB::select("SELECT * from get_rekap_pph21_final('".$request->startdate."','".$request->enddate."','".$request->branch_id."')")->get();
                $pph21Final = [];
   ...
   public function generate_slip_payroll($id){
        ...
        $data['salary'] = DB::table('take_home_pay')
                            ->select('take_home_pay.*','employees.name as employee_name','branches.name as branch_name','position.position_name')
                            ->leftJoin('employees','employees.id','=','take_home_pay.employee_id')
