                            ...
                            <div class="row">
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <select class="form-control select"  id="work_type" name="work_type">
                                        ...
                                        <option value="61" {{($employee->work_type == '61') ? 'selected' : ''}}>6-1 </option>
                                        <option value="52"  {{($employee->work_type == '52') ? 'selected' : ''}}>5-2 </option>
                                        <option value="41"  {{($employee->work_type == '41') ? 'selected' : ''}}>4-1 </option>
                                        <option value="42"  {{($employee->work_type == '42') ? 'selected' : ''}}>4-2 </option>
                                        {{-- <option value="30"  {{($employee->work_type == '30') ? 'selected' : ''}}>3-0 </option> --}}
                                    </select>
                                </div>
                                <div class="form-group col-md-6">
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <input type="text" name="no_kpj" class="form-control" id="no_kpj" value="{{$employee->no_kpj}}">
                                </div>
                                <hr >
                                <div class="form-group col-md-6">
                    ...
                    <div class="card emp_details">
                        <div class="card-header p-3 d-flex align-items-center justify-content-between">
                            ...
                            <div class="d-flex justify-content-end">
                                <button class="btn btn-primary" id="add-contract"><i class="la la-plus"></i></button>
                            </div>
                        </div>
                        <div class="card-body employee-detail-edit-body fulls-card">
                                    ...
                                    <tbody id="tb-contract">
                                        @foreach ($contract as $pkwt)
                                            <tr>
                                                ...
                                                @if($max_cont_num->max_no == $pkwt->contract_number)
                                                <td class="text-center">
                                                    <a href="javascript:void(0);" class="btn btn-primary edit-data-contract" data-id="{{$pkwt->id}}"><span class="fa fa-edit"></span></a>
                                                    <a href="javascript:void(0);" class="btn btn-danger delete-data-contract" data-id="{{$pkwt->id}}" data-employee_id = "{{$pkwt->employee_id}}"><span class="fa fa-trash"></span></a>
                                                </td>
                                                @else
                                                <td></td>
                                                @endif
                            ...
                            <div class="row border border-3 p-3 mt-4" id="form-add-contract" hidden>
                                <div class="form-group col-md-6">
                                    <label for="name" class="form-label">Employee ID</label><span class="text-danger pl-1">*</span>
                                    <input class="form-control" id="add-employee_id-contract"  type="text" value="{{ $employee->no_employee }}">
                                    <input type="hidden" id="add-employee-id"  type="text" value="{{ $employee->id }}">
                                </div>
                                <div class="form-group col-md-6">
                                    <label for="name" class="form-label">Employee Name</label><span class="text-danger pl-1">*</span>
                                    <input class="form-control" id="add-employee_name-contract"  type="text" value="{{ $employee->name }}">
                                </div>
                                <div class="form-group col-md-6">
                                    <label for="name" class="form-label">Start date</label><span class="text-danger pl-1">*</span>
                                    <input class="form-control" id="add-start-date-contract"  type="date" value="">
                                </div>
                                <div class="form-group col-md-6">
                                    <label for="phone" class="form-label">End date</label><span class="text-danger pl-1">*</span>
                                    <input class="form-control" id="add-end-date-contract" type="date" id="phone" value="">
                                </div>
                                <div class="form-group col-md-6">
                                    <label for="phone" class="form-label">Type</label><span class="text-danger pl-1">*</span>
                                    <select class="form-control form-select" name="contract_type" id="add-type-contract">
                                ...
                                <div class="col-md-12">
                                    <button class="btn btn-info float-end" id="cancel-add-contract">Cancel</button>
                                    <button class="btn btn-primary float-end me-1" id="btn-save-contract">Save</button>
                                </div>
                            </div>
                        </div>
                    </div>
...
@include('includes.modal.employee-contract-modal')
@endsection
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $('#add-contract').on('click',function(e){
                e.preventDefault();
                $('#form-add-contract').attr('hidden',false);
            })
            $('#cancel-add-contract').on('click',function(e){
                e.preventDefault();
                $('#form-add-contract').attr('hidden',true);
            })
            $('#btn-save-contract').on('click', function(e){
                e.preventDefault();
                var employee_id = $('#add-employee-id').val()
                var startdate = $('#add-start-date-contract').val()
                var enddate = $('#add-end-date-contract').val()
                var type = $('#add-type-contract').val()
                ...
                $.ajax({
                    ...
                    success : function(respon){
                        if (respon.status == 'success'){
                            var table = '';
                            $.each(respon.contract.contract,function(key,val){

                                if (respon.contract.max_contract.max_no === val.contract_number){
                                    var link = `
                                    <a href="javascript:void(0)" class="btn btn-primary edit-data-contract" data-id="`+val.id+`"><span class="fa fa-edit"></span></a>
                                    <a href="javascript:void(0);" class="btn btn-danger delete-data-contract" data-id="`+val.id+`" data-employee_id = "`+val.employee_id+`"><span class="fa fa-trash"></span></a>`;
                                }else{
                                    var link ='';
                                }
                                table +=`<tr>
                                            <td class="text-center">`+val.contract_number+`</td>
                                            <td>`+val.no_employee+`</td>
                                            <td class="text-center">`+val.startdate+`</td>
                                            <td class="text-center">`+val.enddate+`</td>
                                            <td class="text-center">`+val.contract_type+`</td>
                                            <td class="text-center">`+link+`</td>
                                        </tr>`;
                            })
                            $('#form-add-contract').attr('hidden',true);
                            $('#add-start-date-contract').val('')
                            $('#add-end-date-contract').val('')
                            $('#tb-contract').html(table);
                            swal.fire({
                                icon : respon.status,
                                text : respon.msg
                            })
            ...
            $(document).on('click','.edit-data-contract',function(e){
                e.preventDefault();
                var id = $(this).attr('data-id');
                $('#ContractModal').modal('show')
                $.ajax({
                    url : "{{route('edit-log_contract')}}",
                    type : 'post',
                    data : {id : id },
                    dataType : 'json',
                    beforeSend : function(){
                    ...
                    success : function(respon){
                        $('#id_contract').val(respon.id)
                        $('#edit-employee-id').val(respon.employee_id)
                        $('#edit-employee_id-contract').val(respon.no_employee)
                        $('#edit-employee_name-contract').val(respon.name)
                        $('#edit-start-date-contract').val(respon.startdate)
                        $('#edit-end-date-contract').val(respon.enddate)
                        $('#edit-type-contract').val(respon.contract_type)
                    }
                })
                
            })
            $('#formUpdateContract').on('submit',function(e){
                e.preventDefault();
                var data = $('#formUpdateContract').serialize();
                $.ajax({
                    url : "{{route('update-contract')}}",
                    type : 'post',
                    data : data,
                    dataType : 'json',
                    beforeSend : function(){
            ...
            $(document).on('click','.delete-data-contract', function(e){
                e.preventDefault()
                var id          = $(this).attr('data-id')
