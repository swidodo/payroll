            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-striped table-bordered table-sm w-100" id="tblTarif-category-C">
                        <thead>
                            <tr>
                                <th colspan="5" class="text-center" style="background:#FE8C51">TER DAILY</th>
                            </tr>
                            <tr>
                                <th colspan="5" class="text-center">K/3</th>
                            </tr>
                            <tr>
                        ...
                        <tbody>
                            @foreach ($category_harian as $h)
                                <tr>
                                    <td class="text-center">{{ ($h->start_value !='' && $h->start_value !=null ) ? number_format($h->start_value) :''}} </td>
                                    <td class="text-center">-</td>
                                    <td class="text-center">{{ ($h->end_value !='' && $h->end_value !=null ) ? number_format($h->end_value) : '' }}</td>
                                    <td class="text-center">{{ $h->tarif.' %' }}</td>
                                    <td class="text-center">
                                        <div class="dropdown dropdown-action">
                                            <a href="#" class="action-icon dropdown-toggle" data-bs-toggle="dropdown" aria-expanded="false"><i class="material-icons">more_vert</i></a>
                                            <div class="dropdown-menu dropdown-menu-right">
                                                <a  data-id='{{$h->id}}' class="dropdown-item edit_ter fw-bold" href="javascript:void(0)"><i class="fa fa-pencil m-r-5 me-1"></i> Edit</a>
                                            </div>
                                        </div>
                                    </td>
                                </tr>
        ...
        $(document).on('click','.edit_ter', function(e){
            ...
            $.ajax({
                ...
                success : function(respon){
                    ...
                    if (respon.category == 'H'){
                        $('#heading').html('EDIT TER DAILY')
                    }else{
                ...
                success : function(respon){
                    ...
                    if(respon.status == 'success'){
                        ...
                        swal.fire({
                            ...
                            window.location.href = "{{route('ter-category-daily-rate')}}";
                        })
                    }else{
                        swal.fire({
                            icon : respon.status,
