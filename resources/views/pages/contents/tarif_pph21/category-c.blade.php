            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-striped table-bordered table-sm w-100" id="tblTarif-category-C">
                        <thead>
                            <tr>
                                <th colspan="5" class="text-center" style="background:#FE8C51">CATEGORY C</th>
                            </tr>
                            <tr>
                        ...
                        <tbody>
                            @foreach ($category_c as $c)
                                <tr>
                                    <td class="text-center">{{ ($c->start_value !='' && $c->start_value !=null ) ? number_format($c->start_value) :''}} </td>
                                    <td class="text-center">-</td>
                                    <td class="text-center">{{ ($c->end_value !='' && $c->end_value !=null ) ? number_format($c->end_value) : '' }}</td>
                                    <td class="text-center">{{ $c->tarif.' %' }}</td>
                                    <td class="text-center">
                                        <div class="dropdown dropdown-action">
                                            <a href="#" class="action-icon dropdown-toggle" data-bs-toggle="dropdown" aria-expanded="false"><i class="material-icons">more_vert</i></a>
                                            <div class="dropdown-menu dropdown-menu-right">
                                                {{-- <a  data-id='{{$c->id}}' class="dropdown-item edit_ter fw-bold" href="javascript:void(0)"><i class="fa fa-pencil m-r-5 me-1"></i> Edit</a> --}}
                                            </div>
                                        </div>
                                    </td>
                                </tr>
                ...
                success : function(respon){
                    ...
                    if(respon.status == 'success'){
                        ...
                        swal.fire({
                            ...
                            window.location.href = "{{route('ter-categoryC-monthly-rate')}}";
                        })
                    }else{
                        swal.fire({
                            icon : respon.status,
